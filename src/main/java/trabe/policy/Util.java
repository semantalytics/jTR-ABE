...
public class Util {
    ...
    public static BigInteger unsignedToBigInteger(final long l) {
        final BigInteger bi = BigInteger.valueOf(l);
        return l >= 0 ? bi : bi.add(BI_2_64);
    }

    public static String bit_marker_flexint(final String attribute, final int bit, final boolean on) {
        return bit_marker(attribute, FLEXINT_TYPE, FLEXINT_MAXBITS, bit, on);
    }

    public static String bit_marker_geohash(final String attribute, final int bit, final boolean on) {
        return bit_marker(attribute, GEOHASH_TYPE, GEOHASH_MAXBITS, bit, on);
    }

    private static String bit_marker(final String attribute,
                                     final String type,
                                     final int maxBits,
                                     final int bit,
                                     final boolean on) {
